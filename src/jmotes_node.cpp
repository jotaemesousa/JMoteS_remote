...
int main(int argc, char *argv[])
{
    ...
    while(n.ok())
    {
        if((ros::Time::now() - last_time).toNSec() < 100000000)
        {
            ...
            remote_msg.buttons.push_back((buttons & L1_BUTTON) >> 0);
            remote_msg.buttons.push_back((buttons & L2_BUTTON) >> 1);
            remote_msg.buttons.push_back((buttons & R1_BUTTON) >> 2);
            remote_msg.buttons.push_back((buttons & R2_BUTTON) >> 3);
